...
class Airport:
    ...
    def _compute_structured_opex_snapshot(self):
        """Compute OPEX using structured drivers for the current state (no shocks or scenario tweaks)."""
        P_t = max(0.0, float(getattr(self, "traffic", 0.0)))
        ARV_t = max(1.0, float(getattr(self, "asset_replacement_value", 1.0)))
        SQM_t = float(getattr(self, "non_aero_sqm", 0.0))
        pax_per_flight = max(1, int(getattr(self, "pax_per_movement", 120)))
        M_t = P_t / pax_per_flight

        wage_growth_t = float(getattr(self, "wage_growth", 0.0))
        inflation_t = float(getattr(self, "inflation_assumption", 0.0))
        price_kWh_t = float(getattr(self, "price_per_kwh", 0.20))

        aer_charge = float(getattr(self, "aeronautical_charge", 12.0))
        non_aero_spend = float(getattr(self, "non_aero_spend_per_pax", 9.0))
        cargo_charge = float(getattr(self, "cargo_charge_per_tonne", 30.0))
        cargo_tonnes = float(getattr(self, "cargo_tonnes", 0.0))
        total_revenue_guess = (
            P_t * aer_charge
            + P_t * non_aero_spend * (SQM_t / 5000.0 if SQM_t else 1.0)
            + cargo_tonnes * cargo_charge
        )

        StaffCost = (float(getattr(self, "hr_base", 2_000_000.0))
                     + float(getattr(self, "hr_per_pax", 0.6)) * P_t
                     + float(getattr(self, "hr_per_movement", 30.0)) * M_t) * (1 + wage_growth_t)
        MaintCost = float(getattr(self, "maint_pct_arv", 0.010)) * ARV_t + float(getattr(self, "maint_per_movement", 10.0)) * M_t
        kwh_terminal = float(getattr(self, "kwh_per_pax", 2.0)) * P_t * float(getattr(self, "energy_intensity_factor", 1.0))
        kwh_retail = SQM_t * float(getattr(self, "kwh_per_sqm", 12.0))
        EnergyCost = (kwh_terminal + kwh_retail) * price_kWh_t
        SecComp = float(getattr(self, "sec_base", 1_000_000.0)) + float(getattr(self, "sec_per_pax", 0.2)) * P_t + float(getattr(self, "sec_per_movement", 8.0)) * M_t
        ITCost = float(getattr(self, "it_base", 500_000.0)) + float(getattr(self, "it_per_pax", 0.05)) * P_t + float(getattr(self, "it_pct_arv", 0.001)) * ARV_t
        Insurance = float(getattr(self, "ins_pct_arv", 0.001)) * ARV_t + float(getattr(self, "ins_pct_revenue", 0.005)) * max(0.0, total_revenue_guess)
        tonnes_CO2 = (float(getattr(self, "co2_per_pax_kg", 2.0)) * P_t) / 1000.0
        Env = tonnes_CO2 * float(getattr(self, "carbon_price", 80.0)) + float(getattr(self, "noise_per_movement", 0.0)) * M_t
        ConcessionFee = float(getattr(self, "concession_fee_pct", 0.0)) * max(0.0, total_revenue_guess)
        HandlingSupport = float(getattr(self, "handling_per_movement", 0.0)) * M_t
        Other = float(getattr(self, "other_base", 1_000_000.0)) * (1 + inflation_t)

        opex_struct = (
            StaffCost + MaintCost + EnergyCost + SecComp + ITCost +
            Insurance + Env + ConcessionFee + HandlingSupport + Other
        )
        return float(opex_struct)

    def recalc_initial_baseline(self):
        """Recalculate baseline ARV, OPEX and non-aero spend so the Setup screen shows realistic values."""
        cap = max(1, int(getattr(self, "capacity_pax", 1)))
        arv = float(getattr(self, "asset_replacement_value", 200_000_000.0))
        # Keep €/capacity-pax ratio while respecting the entered capacity
        self.asset_replacement_value = (arv / max(1, cap)) * cap

        P = float(getattr(self, "traffic", 0.0))
        # EU-ish anchors by size
        if P < 5_000_000:
            self.non_aero_spend_per_pax = 8.5
            tgt_opex_per_pax, tgt_pct_arv = 24.0, 0.025
        elif P < 15_000_000:
            self.non_aero_spend_per_pax = 10.5
            tgt_opex_per_pax, tgt_pct_arv = 18.0, 0.020
        else:
            self.non_aero_spend_per_pax = 12.0
            tgt_opex_per_pax, tgt_pct_arv = 15.0, 0.016

        util = P / max(1, cap)
        util_adj = min(1.10, max(0.90, 0.95 + 0.2 * (util - 0.7)))
        self.non_aero_spend_per_pax *= util_adj

        # Calibrate drivers if available
        if hasattr(self, "calibrate_costs"):
            try:
                self.calibrate_costs(target_opex_per_pax=tgt_opex_per_pax, target_opex_pct_arv=tgt_pct_arv)
            except TypeError:
                try:
                    self.calibrate_costs(tgt_opex_per_pax, tgt_pct_arv)
                except Exception:
                    pass

        # Structured snapshot for current state
        self.opex = self._compute_structured_opex_snapshot()
        self.total_opex = self.opex
        self.opex_ratio = self.opex / max(1.0, float(self.asset_replacement_value))
...
# NEW: track whether baseline has been explicitly applied at least once
...
if st.session_state.airport.strategy is None:
    with tab_setup:
        # --- Baseline input step (new) ---
        st.header("Setup: Enter Current Baseline")
        a = st.session_state.airport
        cap_input = st.number_input(
            "Current Passenger Capacity (annual, pax):",
            min_value=0, step=10_000, value=int(a.capacity_pax)
        )
        pax_input = st.number_input(
            "Current Number of Passengers (annual, pax):",
            min_value=0, step=10_000, value=int(a.traffic)
        )
        cargo_input = st.number_input(
            "Current Number of Cargo Tonnes (annual, tonnes):",
            min_value=0, step=1_000, value=int(a.cargo_tonnes)
        )
        ...
        if st.button("Apply baseline"):
            ...
            # NEW: recalc ARV, OPEX and non-aero spend to make setup metrics dynamic
            a.recalc_initial_baseline()
            st.session_state.baseline_set = True
            st.success("Baseline updated. Financials recalibrated to your baseline.")

        # --- Status + Strategic Planning (as before) ---
        ...
        # Require at least one baseline apply before starting (optional; comment out if not desired)
        disabled_start = not st.session_state.baseline_set
        if disabled_start:
            st.info("Enter values above and click **Apply baseline** to enable the simulation start.")

        if st.button("Start Simulation", disabled=disabled_start):
    ...
    with tab_decisions:
        ...
        if st.button("Simulate Year"):
            ...
            year_data = {
                ...
                'Terminal Utilization': (st.session_state.airport.traffic / st.session_state.airport.capacity_pax) * 100 if st.session_state.airport.capacity_pax > 0 else 0.0,
                'Runway Utilization': (st.session_state.airport.current_movements / st.session_state.airport.runway_capacity_movements) * 100 if st.session_state.airport.runway_capacity_movements > 0 else 0.0,
